...
// console.log(johnDoe.friends);

// Exercise 2: Use the filter method to get all the people who live in New York
const peopleInNewYork = data.filter(person => person.address.city === "New York");
// console.log(peopleInNewYork);


// Exercise 3: Use the filter method to get all the people who are older than 30
const PeopleOver30YearsOld = data.filter(person => person.age > 30)
// console.log(PeopleOver30YearsOld);
...
const johnDoeFriendNames = data.filter(person => person.name == "John Doe").map(person => person.friends.map(friend => friend.name)).flat();
...
const fullAddresses = data.map(person => person.address);
...
const hobbies = data.map(person => person.hobbies).flat();
console.log(hobbies);
